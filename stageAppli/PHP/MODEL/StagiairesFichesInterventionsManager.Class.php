...
class StagiairesFichesInterventionsManager 
{
	...
	public static function getStagiaireByFiche($fiche)
    {
        $db = DbConnect::getDb();
        $liste = [];
        $fiche = (int) $fiche;
        $q = $db->query("SELECT DISTINCT idUser FROM amb_stagiairesfichesinterventions WHERE idFicheIntervention = ".$fiche );
        while ($donnees = $q->fetch(PDO::FETCH_ASSOC))
        {
            if ($donnees != false)
            {
                $liste[] = StagiairesFichesInterventionsManager::findById($donnees['idUser']);
            }
        }
        return $liste;
    }
